...
const PIPELINE_COLUMNS: { stage: ApplicationStage; title: string; color: string }[] = [
...
export function KanbanPipeline({
  ...
  // Modal states for Hiring confirmation
  const [hiringModalApp, setHiringModalApp] = useState<Application | null>(null);
  const [confirmingHire, setConfirmingHire] = useState(false);

  // Modal states for Rejection confirmation
  ...
  async function handleTransition(applicationId: string, newStage: ApplicationStage) {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error?.message || "Failed to update application stage");
      }
  ...
  async function handleScheduleInterviewSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!interviewModalApp || !interviewDate || !interviewTime) return;
    ...
    try {
      ...
      const res = await fetch(`/api/applications/${interviewModalApp.id}/interviews`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleHireSubmit() {
    if (!hiringModalApp) return;
    setConfirmingHire(true);
    setErrorMsg(null);

    try {
      const res = await fetch(`/api/applications/${hiringModalApp.id}/stage`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ new_stage: "hired" }),
      });
      const data = await res.json();

      if (!res.ok || !data.success) {
        throw new Error(data.error?.message || "Hiring capacity reached or failed");
      }
  ...
  async function handleRejectSubmit() {
    ...
    try {
      const res = await fetch(`/api/applications/${rejectionModalApp.id}/stage`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ new_stage: "rejected" }),
      });
      const data = await res.json();
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <span className="text-xs font-semibold text-slate-600">Filter Position:</span>
          <select
            value={currentJobId}
            onChange={(e) => {
              setCurrentJobId(e.target.value);
              router.push(e.target.value ? `/recruiter/pipeline?job_id=${e.target.value}` : "/recruiter/pipeline");
            }}
          ...
          >
            <option value="">All Assigned Positions ({jobs.length})</option>
            {jobs.map((job) => (
        ...
        <div className="text-xs text-slate-500">
          Showing <strong>{filteredApps.length}</strong> candidates in pipeline
        </div>
      </div>

      {/* Global Error Notice if any action fails */}
      {errorMsg && (
      ...
      <div className="overflow-x-auto pb-4">
        <div className="flex gap-4 min-w-[1400px]">
          {PIPELINE_COLUMNS.map((col) => {
            const columnApps = filteredApps.filter((a) => a.stage === col.stage);
                    ...
                    columnApps.map((app) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Quick Action Buttons according to state machine */}
                            ...
                            {isLoading ? (
                              ...
                              <>
                                ...
                                {app.stage === "applied" && (
                                  <>
                                    <button
                                      onClick={() => handleTransition(app.id, "shortlisted")}
                                ...
                                {app.stage === "interview" && (
                                  <>
                                    <button
                                      onClick={() => handleTransition(app.id, "offer")}
                                    ...
                                    >
                                      <Gift className="w-3 h-3" /> Extend Offer
                                    </button>
                                    <button
                                      onClick={() => setRejectionModalApp(app)}
                                      className="py-1 px-2 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-700 font-semibold text-[11px] transition"
                                ...
                                {app.stage === "offer" && (
                                  <>
                                    ...
                                    >
                                      <CheckCircle2 className="w-3 h-3" /> Mark Hired
                                    </button>
                                    <button
                                      onClick={() => setRejectionModalApp(app)}
                                      className="py-1 px-2 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-700 font-semibold text-[11px] transition"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Candidate: {interviewModalApp.candidate?.full_name}
                </p>
              </div>
            </div>

            <form onSubmit={handleScheduleInterviewSubmit} className="space-y-4 pt-2">
              <div>
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  Duration is strictly 60 minutes. End time will be calculated automatically.
                </p>
              </div>
          ...
          <div className="bg-white max-w-md w-full rounded-2xl shadow-xl border border-slate-200 p-6 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                Are you sure you want to mark <strong>{hiringModalApp.candidate?.full_name}</strong> as Hired?
                This action is final. If the total hired count reaches the job openings, the position will automatically close to new applicants.
              </p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white max-w-md w-full rounded-2xl shadow-xl border border-slate-200 p-6 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                Are you sure you want to reject <strong>{rejectionModalApp.candidate?.full_name}</strong>?
                This decision will be recorded in the audit history and a polite rejection notification email will be queued.
              </p>
            </div>
